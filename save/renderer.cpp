...
Renderer::Renderer() {
    ...
    s = ecs.update_signature<Animator>();
    collectors.push_back(Collector{s, false});
}
...
void Renderer::call() {
    ...
    Transform2D& player_tf = ecs.get_component<Transform2D>(is.player_entity);
    ...
    if(player_tf.position.x < center_region.x) {
        new_pos.x += player_tf.position.x - center_region.x;
    } else if(player_tf.position.x > center_region.z) {
        new_pos.x += player_tf.position.x - center_region.z;
    }

    if(player_tf.position.y < center_region.y) {
        new_pos.y += player_tf.position.y - center_region.y;
    } else if(player_tf.position.y > center_region.w) {
        new_pos.y += player_tf.position.y - center_region.w;
    }
    ...
    for(uint32_t entity : collectors[2].entities) render_animation(entity);
    //
    render_points();
...
void Renderer::render_animation(uint32_t entity) {
    Transform2D& ct = ecs.get_component<Transform2D>(camera);
    Camera2D& cc = ecs.get_component<Camera2D>(camera);
    ...
    Transform2D& transform = ecs.get_component<Transform2D>(entity);
    Animator& animator = ecs.get_component<Animator>(entity);

    animator.current_animation = 0;

    animator.current_time += core.delta_time;

    //

    float time = 0.0f;
    for(uint32_t frame_i = 0; frame_i < animator.animations[animator.current_animation].frames.size(); ++frame_i) {
        auto& frame = animator.animations[animator.current_animation].frames[frame_i];

        time += frame.duration;
    }

    uint32_t f = 0;
    float t = fract(animator.current_time / time) * time;
    float accum = 0.0f;
    for(uint32_t frame_i = 0; frame_i < animator.animations[animator.current_animation].frames.size(); ++frame_i) {
        auto &frame = animator.animations[animator.current_animation].frames[frame_i];
    ...
    vec4 range = animator.animations[animator.current_animation].frames[f].regions[0];
    //
    ...
    auto& texture = core.textures[animator.texture];
    ...
    glUniformMatrix4fv(1, 1, false, &view[0][0]);
    glUniformMatrix4fv(2, 1, false, &model[0][0]);
    glUniform2fv(3, 1, &animator.size.x); // size
    glUniform2fv(4, 1, &animator.rel.x); // offset
    glUniform4fv(5, 1, &range.x); // range
    glUniform3fv(6, 1, &light.x);
