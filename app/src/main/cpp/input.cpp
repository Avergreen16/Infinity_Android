...
void Input_system::call() {
    ...
    if(pointers.size() >= 2) {
        ...
        mat2 rot = transpose(glm::rotate(delta_rotate, vec3(0.0f, 0.0f, 1.0f)));

        vec2 pivot = camera_transform.orientation * (((prev_avg / vec2(width, height) * 2.0f - 1.0f) * vec2(1.0f, -1.0f)) * vec2(1.0f, height / width) / camera_camera.scale) + camera_transform.position;

        camera_transform.position = rot * (camera_transform.position - pivot) + pivot;
        camera_transform.orientation = rot * camera_transform.orientation;
    } else {
